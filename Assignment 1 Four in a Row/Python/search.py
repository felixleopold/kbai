...
class GameNode:
    def __init__(self, board: Board, player_to_move: int, heuristic: Heuristic, move: Optional[int] = None, depth: int = 0):
        self.board = board
    ...
    def is_terminal(self, game_n: int) -> bool:
        """
...
def expand_children(node: GameNode) -> List["GameNode"]:
    ...
    for col in range(node.board.width):
        if node.board.is_valid(col):
            ...
            child_node = GameNode(new_board, childs_player_to_move, node.heuristic, col, node.depth + 1) # col is the col of the parent
            children.append(child_node)
    return children

def minimax(node: GameNode, depth: int, game_n: int, eval_player: int) -> int:
    """
    ...
    if node.is_terminal(game_n) or depth == 0:
        return node.heuristic.evaluate_board(eval_player, node.board)
    
    children = expand_children(node)
    
    if node.player_to_move == eval_player:
        # Maximizing player
        best_value = -np.inf
        for child in children:
            child_value = minimax(child, depth - 1, game_n, eval_player)
...
def test_is_terminal():
    ...
    empty_node = GameNode(empty_board, 1, heuristic)
    print_board_state(empty_board, "Empty Board")
    result = empty_node.is_terminal(game_n)
    ...
    win_node = GameNode(win_board, 2, heuristic)  # Player 2's turn but game is over
    print_board_state(win_board, "Player 1 Wins")
    result = win_node.is_terminal(game_n)
    print(f"Is terminal: {result}")
    ...
    draw_node = GameNode(draw_board, 1, heuristic)
    print_board_state(draw_board, "Draw Board")
    result = draw_node.is_terminal(game_n)
    print(f"Is terminal: {result}")
    ...
    win2_node = GameNode(win2_board, 1, heuristic)
    print_board_state(win2_board, "Player 2 Wins")
    result = win2_node.is_terminal(game_n)
    print(f"Is terminal: {result}")
...
def test_expand_children():
    ...
    partial_node = GameNode(partial_board, 2, heuristic)
...
def test_minimax():
    ...
    best_value = minimax(empty_node, depth, game_n, 1)
    ...
    print(f"\nChildren analysis:")
    for i, child in enumerate(children):
        child_value = minimax(child, depth-1, game_n, 1)
    ...
    near_win_node = GameNode(near_win_board, 1, heuristic)
    print_board_state(near_win_board, "Root Board")
    
    best_value = minimax(near_win_node, depth, game_n, 1)
    print(f"Best value for Player 1: {best_value}")
    ...
    near_loss_node = GameNode(near_loss_board, 1, heuristic)
    print_board_state(near_loss_board, "Root Board")
    
    best_value = minimax(near_loss_node, depth, game_n, 1)
    print(f"Best value for Player 1: {best_value}")
...
def test_complete_minimax_player():
    ...
    def minimax_player_move(board, player_id, depth, game_n, heuristic):
        ...
        root = GameNode(board, player_id, heuristic)
        ...
        for child in children:
            child_value = minimax(child, depth-1, game_n, player_id)
